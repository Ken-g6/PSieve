 ...
 * With improvements by Ken Brazier August-September 2009.
...
#define BSFQ(MPOS,KPOS,ID)          asm volatile \
            ( 		"bsfl	%[k0l], %[pos]	\n" \
                ...
                : "cc" )
#else
// If anyone wants to compile on some non-x86 platform...
...
void app_init(void)
{
  ...
  xkmax[1] = xkmax[0];
  if(xkmax[0] < (UINT64_C(1)<<32) && pmax < xkmax[0]*(UINT64_C(1)<<32)) {
#ifdef __x86_64__
    ...
    printf("bsf takes %lu; mul takes %lu; using ", bsftime, multime);
    if(((unsigned int)a != c) && bsftime <= multime) {
      printf("standard algorithm.\n");
      sse2_in_range = 0;
    } else {
      printf("alternate algorithm.\n");
      sse2_in_range = 1;
    }
#else
    sse2_in_range = 1;
    ...
    // Reset xkmax for the usual SSE2 algorithm.
    xkmax[0] = kmax+1;
    xkmax[1] = xkmax[0];
  }
#endif
...
#else
#ifdef EMM
    // Find the odd value of K or P-K and check it;
    // Also save the even value in T.
    {
...
#define TESTEVENP(NUM,NUMID) \
    kpos = T[NUM]; \
    BSFQ(mpos, kpos, NUMID"EP"); \
    kpos >>= mpos; \
    if (kpos <= kmax && kpos >= kmin && n+mpos <= nmax) \
    test_factor(P[NUM],kpos,n+mpos,2*(((unsigned int)K[NUM])&1)-1)

    // Check the even values in T.
    {
      uint64_t kpos;
      unsigned int mpos;
      ...
      if(sse2_in_range)
        ...
        for(i=0; i < APP_BUFLEN; i+=2) {
          ...
          if ((bitst & 0x80)) {
            ...
            if (kkpos <= kmax && pos >= kmin && mpos < nstep)
              test_factor(P[i],kpos,n+mpos,+1);
          }
          if ((bitst & 0x8000)) {
            uint64_t kpos;
...
// A macro of what would go in a loop but for the unique IDs needed:
#define TESTEVEN(NUM,NUMID) \
          kpos = T[NUM]; \
          BSFQ(mpos, kpos, NUMID"E"); \
          kpos >>= mpos; \
          if (kpos <= kmax && kpos >= kmin && mpos < nstep) \
            test_factor(P[NUM],kpos,n+mpos,2*(((unsigned int)K[NUM])&1)-1)
